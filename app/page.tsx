...
const media: Record<number, Media> = {
  0: { provider: 'youtube', src: 'https://www.youtube-nocookie.com/embed/TIuGFvr5yhU?rel=0' },
  1: { provider: 'bandcamp', src: 'https://bandcamp.com/EmbeddedPlayer/album=4027376985/size=large/bgcol=111111/linkcol=f4f4f1/transparent=true/' },
  2: { provider: 'bandcamp', src: 'https://bandcamp.com/EmbeddedPlayer/album=3987444618/size=large/bgcol=111111/linkcol=f4f4f1/transparent=true/' },
  3: { provider: 'youtube', src: 'https://www.youtube-nocookie.com/embed/jgBRVmc9kFM?rel=0' },
  4: { provider: 'bandcamp', src: 'https://bandcamp.com/EmbeddedPlayer/album=1823207988/size=large/bgcol=111111/linkcol=f4f4f1/transparent=true/' },
};

const details: Record<number, string> = {
  0: '2025 · PRODUCER / GUITAR / INSTRUMENTATION',
  1: '2023–2026 · MASTERING',
  2: '2021 · PRODUCER / MIX',
  3: '2021 · CO-PRODUCTION',
  4: '2020 · MIX / PRODUCTION',
};

const localImage = (index: number) => {
...
export default function Home() {
  const [stage, setStage] = useState<{ index: number; face: 'info' | 'media' } | null>(null);

  return <main className="project-field" aria-label="Alastair McNeill project archive">
    <section className="project-grid">
      {titles.map((title, index) => {
        const face = stage?.index === index ? stage.face : 'image';
        return <article key={title} className={`slice ${face === 'info' ? 'is-info' : ''} ${face === 'media' ? 'is-media' : ''}`}>
        <div className="slab">
          <button className="face front" type="button" onClick={() => setStage({ index, face: 'info' })} aria-label={`Show information for ${title}`}><img src={localImage(index)} alt="" loading={index === 0 ? 'eager' : 'lazy'} fetchPriority={index === 0 ? 'high' : 'auto'} decoding="async" /></button>
          <button className="face right" type="button" onClick={() => media[index] ? setStage({ index, face: 'media' }) : setStage(null)} aria-label={media[index] ? `Show media for ${title}` : `Show image for ${title}`}><strong>{title}</strong>{details[index] && <small>{details[index]}</small>}</button>
          <span className="face left neutral" aria-hidden="true" />
          <span className="face top neutral" aria-hidden="true" />
          <span className="face bottom neutral" aria-hidden="true" />
          <span className={`face back ${face === 'media' && media[index] ? 'media-face' : 'neutral'}`}>{face === 'media' && media[index] && <iframe src={media[index].src} title={`${title} media`} allow="encrypted-media; picture-in-picture" allowFullScreen />}</span>
        </div>
        {face === 'info' && media[index] && <button className="media-advance" type="button" onClick={() => setStage({ index, face: 'media' })} aria-label={`Show media for ${title}`} />}
      </article>})}
    </section>
  </main>;
}
